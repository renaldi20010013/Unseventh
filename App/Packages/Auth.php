...
class Auth
{
    ...
    public static function attempt(array $credentials = [], $validator = null) : bool
    {
        ...
        if (self::validate($credentials)) {
            $user = DB::table('registrasi');
            $field = isset($credentials['username']) ? 'username' : 'username';
            $user = $user->where($field, isset($credentials['username']) ? $credentials['username'] : $credentials['username']);
            $user = $user->first();
